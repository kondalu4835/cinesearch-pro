...
import { getTrending, getNowPlaying, getTopRated, getUpcoming, discover } from '../utils/tmdb';
      ...
      <div className="relative z-10 max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 pt-32 w-full">
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-2xl">
          ...
          <div className="flex items-center gap-4 mb-5 flex-wrap">
            ...
            {movie.adult === false && <span className="text-xs border border-dark-600 text-dark-400 px-2 py-0.5 rounded">PG</span>}
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <Link to={`/movie/${movie.id}`} className="btn-primary text-base px-8 py-3.5">
              View Details
            </Link>
            <Link to={`/movie/${movie.id}`} className="btn-secondary text-base px-8 py-3.5">
              ▶ Watch Trailer
            </Link>
          </div>
        </motion.div>
...
const Home = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [t, n, top, u, b, ta] = await Promise.all([
          getTrending('movie', 'week'),
          getNowPlaying(),
          getTopRated(),
          getUpcoming(),
          discover({ with_original_language: 'hi', sort_by: 'popularity.desc' }),
          discover({ with_original_language: 'ta', sort_by: 'popularity.desc' }),
        ]);
        setTrending(t.results   || []);
        setNowPlaying(n.results || []);
        setTopRated(top.results || []);
        setUpcoming(u.results   || []);
        setBollywood(b.results  || []);
        setTamil(ta.results     || []);
  ...
  return (
    <div className="bg-dark-950 min-h-screen">
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <MovieRow title="Bollywood"                icon="🇮🇳" movies={bollywood}   loading={loading} viewAllLink="/movies?lang=hi" />
        <MovieRow title="Kollywood"                icon="🎬" movies={tamil}       loading={loading} viewAllLink="/movies?lang=ta" />
